				...
				if (partString.compare(csv[i][j]) == 0) {
					...
					if (rfid_event1 == 1)
					{
						...
						if (rfid_status == 2) {
							...
							csv[i][j + 10] = ofToString(minutesum[0]);       
			...
			for (int j = 0; j < csv.getNumCols(i); j++) {
				...
				if ((rfid_status % 2 == 0) && (str4.compare(off) == 0)){
					...
					if (str2.compare(csv[i][j]) == 0)
					{
						...
						csv[i][1] = "";
						csv[i][2] = "";  
						csv[i][3] = ofToString(load_start_hour);
						csv[i][4] = ofToString(load_start_minute);
						csv[i][5] = ofToString(end_hour);                                // end_hour   
						csv[i][6] = ofToString(end_minute);            // end_minute   
						csv[i][9] = ofToString(minus_end_minute);      // battery time
						//csv[i][10] = ofToString(usertime);
						csv[i][10] = "";
						*/
